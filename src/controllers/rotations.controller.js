...
export const createEntreSemana = async (req, res) => {
  ...
  if (!Nombre || !H_inicio || !H_fin  || !Grupo_id || !Tipo) {
...
export const findAllRegGroupsById = async (req, res) => {
  ...
  try {
    ...
    const allReg = await Rotation.find({Grupo_ID:ID,Tipo:"Entre Semana"}).sort({Nombre:1});
    res.json(allReg);
...
export const findAllRegGroupsByIdFs = async (req, res) => {
  ...
  try {
    ...
    const allReg = await Rotation.find({Grupo_ID:ID,Tipo:"Fin de Semana"}).sort({Nombre:1});
    res.json(allReg);
...
export const actualizarSchema = async (req, res) => {
  ...
  console.log("Entrando a la zona de actualización")
  
  
  try {
    await Rotation.updateMany({_id:id_registro,Nombre:nombreActual},{$set:{Nombre:nuevoNombre,HoraInicio:nuevaHoraInicio,HoraFinal:nuevaHoraFinal}});
