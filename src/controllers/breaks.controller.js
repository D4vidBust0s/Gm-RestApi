/*
  ...
  la base de datos con respecto los descansos de un trabajador
*/

import Breaks from "../models/Breaks";

//Crear un nuevo descanso para un trabajador
export const createPermission = async (req, res) => {

    console.log("ENTRANDO A LA ZONA DE CREACION DE UN DESCANSO");
  ...
  try {
    const newPermission = new Breaks({
    ...
    res.status(500).json({
      message: error.message || "Error intentando crear un nuevo descanso",
    });
  }
};

//Listar todos los descansos de un usurio por su id
export const findAllPermissions = async (req, res) => {
  console.log("ENTRANDO A LA ZONA QUE OBTIENE TODOS LOS LISTADOS DE PERMISOS" );


  try {
    const allPermisions = await Breaks.find();
...
export const findAllPermissionsUser = async (req, res) => {
    ...
    try {
      const allPermisions = await Breaks.find({Id_Empleado:id,Año:año});
 ...
 export const updatePermiso = async (req, res) => {
  ...
  try {
    await Breaks.updateMany({_id:id},{$set:{Nombre:nombre,Año:año,FechaInicio:fechainicio,FechaFinal:fechafinal,Observacion:observacion,Estado:estado,NombreEmpleado:nombreempleado,Id_Empleado:id_empleado}});
...
export const deletePermission = async (req, res) => {
  ...
  try {
    const data = await Breaks.findByIdAndDelete(id);
    res.json({
